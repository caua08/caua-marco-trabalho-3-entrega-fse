...
void esp_hidd_send_gamepad_value(uint16_t conn_id, uint8_t buttons, int8_t x, int8_t y)
{
    uint8_t buffer[3];

    buffer[0] = buttons & 0x0F;
    buffer[1] = (uint8_t)x;
    buffer[2] = (uint8_t)y;

    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,
                        HID_RPT_ID_MOUSE_IN, HID_REPORT_TYPE_INPUT, sizeof(buffer), buffer);
}
